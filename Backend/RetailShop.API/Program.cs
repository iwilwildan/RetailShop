...
using RetailShop.API.Repositories;
...
builder.Services.AddDbContext<RetailShopDbContext>(options => options.UseSqlServer(
...
using (var db = service.CreateScope().ServiceProvider.GetService<RetailShopDbContext>())
{
    db.Database.Migrate();
}
